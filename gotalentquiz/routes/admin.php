...
$authenticate = function ($app, $admin = false) {
    ...
    return function () use ($app, $admin) {
        ...
        if ($admin)
        {
            ...
            {
                $errors['loginerrorr'] = 'Você não tem acesso de administrador.';
                $app->session->set('urlRedirect', $app->request()->getPathInfo());
                $app->flash('errorrs', $errorrs);
        ...
        else if (\SimpleQuiz\Utils\Base\Config::$requireauth)
        {
            ...
            {
                $errors['loginerror'] = 'Você precisa fazer o login para participar de um Quiz.	';
                $app->session->set('urlRedirect', $app->request()->getPathInfo());
...
$app->post("/admin/quiz/", $authenticate($app, true), function() use ($app) {
    ...
    if ( ($quizname !== '') && ($quizdescription !== '') ) {
        ...
        if ($simple->addQuiz($quizmeta)) {
            $app->flash('success', 'Quiz criado com sucesso');

            $app->redirect($app->request->getRootUri().'/admin/');
        } else {
            //problem adding quiz
            $app->flash('error', 'Problema na criação do Quiz');
            $app->redirect($app->request->getRootUri().'/admin/');
        }
    } else {
        //problem with post inputs
        $app->flash('error', 'Problema ao criar o quiz. Alguma entrada está errada');
        $app->redirect($app->request->getRootUri().'/admin/');
    }
...
$app->put("/admin/quiz/", $authenticate($app, true), function() use ($app) {
    ...
    if ( ($quizname !== '') && ($quizdescription !== '') && (ctype_digit($quizid)) ) {
        ...
        if ($simple->updateQuiz($quizmeta)) {
            $app->flash('success', 'Quiz atualizado com sucesso');

            $app->redirect($app->request->getRootUri().'/admin/');
        } else {
            //problem adding quiz
            $app->flash('error', 'Problema ao atualizar o Quiz');
            $app->redirect($app->request->getRootUri().'/admin/');
        }
    } else {
        //problem with post inputs
        $app->flash('error', 'Problema ao atualizar o Quiz. Alguma entrada está errada');
        $app->redirect($app->request->getRootUri().'/admin/');
    }
...
$app->delete("/admin/quiz/", $authenticate($app, true), function() use ($app) {
    ...
    if (ctype_digit($quizid) ) {
        ...
        echo json_encode(array('success' => 'Quiz excluído com sucesso'));
        $app->stop();
...
$app->put("/admin/quiz/:id/", $authenticate($app, true), function($id) use ($app) {
    ...
    if ($quiz->setId($id)) {
        ...
        try {
            ...
            $app->flashnow('success', 'Questão salva com sucesso');
        } catch (Exception $e ) {
...
$app->post("/admin/quiz/:id/", $authenticate($app, true), function($id) use ($app) {
    ...
    if ($quiz->setId($id)) {
        ...
        foreach ($answerarray as $answer) {
            if (trim($answer) == '') {
                $app->flashnow('error', "A resposta não pode ser vazia");
                $app->render('admin/quiz.php', array('quiz' => $quiz, 'categories' => $categories));
                $app->stop();
            }
            if ($i == $correct) {
        ...
        try {
            ...
            $app->flashnow('success', 'Nova questão salva com sucesso');
        } catch (Exception $e ) {
            $app->flashnow('error', 'Um erro ocorreu ao criar uma nova questão');
            $app->flashnow('error', $e->getMessage());
        }
        $quiz->populateUsers();
...
$app->delete("/admin/quiz/:id/", $authenticate($app, true), function($id) use ($app) {
    ...
    if ($quiz->setId($id)) {
        ...
        echo json_encode(array('success' => 'Questão excluída com sucesso'));
        $app->stop();
    }
...
$app->put("/admin/quiz/:quizid/question/:questionid/edit/", $authenticate($app, true), function($quizid,
    ...
    if ($quiz->setId($quizid)) {
        ...
        foreach ($answerarray as $answer) {
            if (trim($answer) == '') {
                $app->flashnow('error', 'Respostas não podem ser vazias');
                $answers = $quiz->getAnswers($questionid);
                $app->render('admin/editanswers.php', array('quizid' => $quizid,'questionid' => $questionid, 'question' => $question, 'answers' => $answers));
                $app->stop();
            }
        ...
        try {
            ...
            $app->flashnow('success', 'Respostas salvas com sucesso');
        } catch (Exception $e ) {
            $app->flashnow('error', 'Um error ocorreu');
        }
        $answers = $quiz->getAnswers($questionid);
        $app->render('admin/editanswers.php', array('quizid' => $quizid,'questionid' => $questionid, 'question' => $question, 'answers' => $answers));
    } else {
